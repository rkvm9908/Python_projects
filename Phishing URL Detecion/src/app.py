...
# Load ML components (Ensure these files exist and are correct)
try:
    ...
    if request.method == "POST":
        ...
        if url:
            ...
            try:
                ...
                print(f"Error saving URL log: {e}")
                db.rollback()
            finally:
                db.close()
...
def api_predict():
    ...
    try:
        features = extract_features(url)
        features_df = features[scaler.feature_names_in_]
...
import subprocess # Training script-ai run panna

DATASET_DIR = "dataset" # Unga datasets irukkura folder
os.makedirs(DATASET_DIR, exist_ok=True)

@app.route("/admin/model-manager")
@login_required
def model_manager():
    if not current_user.is_admin: return redirect(url_for('index'))
    
    # Dataset folder-la ulla files-ai list pannu
    files = os.listdir(DATASET_DIR)
    return render_template("admin/manage.html", files=files)
...
def upload_dataset():
    ...
    if file and file.filename.endswith('.csv'):
        ...
        flash(f"Dataset {file.filename} uploaded!", "success")
...
def train_model():
    ...
    try:
        # Background-la unga training script-ai run pannum
        subprocess.Popen([sys.executable, "src/train_ensemble.py"])
        flash("Training started in background. It may take a few minutes.", "success")
    except Exception as e:
        flash(f"Training failed: {str(e)}", "error")
        
    return redirect(url_for('model_manager'))

@app.route("/admin/delete-dataset/<filename>", methods=["POST"])
@login_required
