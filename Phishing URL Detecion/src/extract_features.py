...
BRAND_LIST = [
    'google', 'facebook', 'instagram', 'whatsapp', 'microsoft', 'apple', 'amazon', 'netflix', 
    'twitter', 'linkedin', 'youtube', 'snapchat', 'tiktok', 'yahoo', 'adobe', 'zoom', 'skype', 
    'pinterest', 'reddit', 'paypal', 'chase', 'wellsfargo', 'citibank', 'hsbc', 'barclays', 
    'bankofamerica', 'visa', 'mastercard', 'amex', 'stripe', 'coinbase', 'binance', 'blockchain', 
    'ebay', 'walmart', 'aliexpress', 'alibaba', 'flipkart', 'shopify', 'target', 'bestbuy', 
    'homedepot', 'costco', 'fedex', 'ups', 'dhl', 'dropbox', 'icloud', 'gmail', 'outlook'
]
...
SELECTED_FEATURE_COLUMNS = [
    ...
    "URLEntropy", "IsPunycode", "IsBrandSpoofed"  
]
...
def extract_features(url):
    ...
    brand_spoof = 0
    normalized_domain = domain_only.replace('0', 'o').replace('1', 'l').replace('3', 'e').replace('4', 'a').replace('5', 's')
    for brand in BRAND_LIST:
        if brand in normalized_domain and brand != domain_only:
            brand_spoof = 1
            break
    features["IsBrandSpoofed"] = brand_spoof
